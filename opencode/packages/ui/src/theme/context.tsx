...
// Forge-branded storage keys. The pre-rename `opencode-*` variants are read
// once and migrated forward (see migrateLegacyThemeKeys below) so existing
// users keep their picked theme across the namespace flip. Writes only ever
// touch the new keys; the legacy ones are removed on first migration.
const STORAGE_KEYS = {
  THEME_ID:        "forge-theme-id",
  COLOR_SCHEME:    "forge-color-scheme",
  THEME_CSS_LIGHT: "forge-theme-css-light",
  THEME_CSS_DARK:  "forge-theme-css-dark",
} as const
...
function migrateLegacyThemeKeys() {
  ...
  try {
    for (const k of Object.keys(STORAGE_KEYS) as Array<keyof typeof STORAGE_KEYS>) {
      if (localStorage.getItem(STORAGE_KEYS[k]) !== null) continue       // already on new key
      const legacy = localStorage.getItem(LEGACY_STORAGE_KEYS[k])
      if (legacy === null) continue
      localStorage.setItem(STORAGE_KEYS[k], legacy)
      localStorage.removeItem(LEGACY_STORAGE_KEYS[k])
    }
...
function read(key: string) {
  ...
  // Cheap idempotent guard — first read of any theme key triggers the one-shot
  // forward-copy of legacy `opencode-*` values. After the first call the flag
  // short-circuits, so the hot path is one Map lookup, not a localStorage scan.
