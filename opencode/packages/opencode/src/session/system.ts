  ...
  Effect.gen(function* () {
    ...
    return Service.of({
      environment: Effect.fn("SystemPrompt.environment")(function* (model: Provider.Model, agent?: Agent.Info) {
        ...
        // Forge scope rules — load-bearing for security. Prepended so it sits
        // at the front of the system prompt (prompt-cache friendly and the
        // model reads it first). Skipped for platform agents like `verify`
