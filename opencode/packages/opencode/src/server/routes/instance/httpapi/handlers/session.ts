  ...
  Effect.gen(function* () {
    ...
    const messages = Effect.fn("SessionHttpApi.messages")(function* (ctx: {
      ...
      if (ctx.query.limit === undefined || ctx.query.limit === 0) {
        ...
        // Forge: strip shell/agent-switched/reasoning/shell-class-tool noise
        // from persisted messages before the FE renders them on refresh.
        // No-op for non-Forge sessions.
        return ForgeRuntimeState.sanitizeMessages(all as any) as typeof all
      }
      ...
      const sanitizedItems = ForgeRuntimeState.sanitizeMessages(page.items as any) as typeof page.items
      if (!page.cursor) return sanitizedItems
      ...
      return HttpServerResponse.jsonUnsafe(sanitizedItems, {
        headers: {
    ...
    const message = Effect.fn("SessionHttpApi.message")(function* (ctx: {
      ...
      // Forge: apply the same sanitization the messages list uses. If the
      // message itself is one we drop entirely (shell / agent-switched /
      // model-switched), surface it as Not Found — from the FE's view these
      // messages do not exist in a Forge session.
      const sanitized = ForgeRuntimeState.sanitizeMessages([msg] as any) as [typeof msg] | []
      if (sanitized.length === 0) return yield* notFound("Message not found")
      return sanitized[0]
    })
