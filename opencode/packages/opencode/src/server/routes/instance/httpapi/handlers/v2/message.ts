  ...
  Effect.gen(function* () {
    ...
    return handlers.handle(
      ...
      Effect.fn(function* (ctx) {
        ...
        // Cursor encoding still uses the ORIGINAL first/last so pagination
        // stays anchored to real timeline positions even when items are
        // dropped — otherwise hiding a leaky message at a page boundary
        // would break the cursor chain.
        const first = messages[0]
        const last = messages.at(-1)
        const items = ForgeRuntimeState.sanitizeMessages(messages as any) as typeof messages
        return {
          items,
          cursor: {
