      ...
      Effect.fn("Agent.state")(function* (ctx) {
        ...
        const agents: Record<string, Info> = {
          ...
          verify: {
            name: "verify",
            description:
              "Forge post-completion verify agent. Runs automatically after the primary agent's turn ends — main agent does not invoke this. Inspects the project's dev container, parses logs, probes backend endpoints if accessible, and loops fix-suggestions until the project runs cleanly. User-facing messages are sanitized: no container/command/log talk ever reaches the FE.",
            mode: "subagent",
            native: true,
            hidden: true,
            permission: Permission.merge(
