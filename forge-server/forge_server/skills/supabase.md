...
- "set up a database" / "add a db" / "i need storage"
- "add login" / "registration" / "sign up" / "auth" / "users"
- "save / store / persist" (data, profiles, posts, anything)
- "magic link" / "OAuth" / "Google sign-in" / "session"
- "connect Supabase" or mentions Supabase by name

This skill handles BOTH the credential connection AND the in-app wiring.
Don't try to scaffold auth or a DB by hand — the platform endpoint at
`/api/supabase/connect` injects env vars into the workspace for you, and this
skill knows how to wire those env vars into each supported stack.
...
- `{"connected": false}` → go to Step 3 (gather creds).
...
### Step 3. Gather credentials — ONE at a time, conversationally
...
> I'll set up Supabase for you. First — do you have a Supabase project
> already, or should I walk you through creating one?
...
> client-side reads — just say "skip".
