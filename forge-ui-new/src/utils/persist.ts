...
// Storage namespaces are Forge-branded so localStorage doesn't leak "opencode"
// to end-users inspecting devtools. Legacy `opencode.*` namespaces remain
// readable via LEGACY_GLOBAL_STORAGE / legacyWorkspaceStorage() so existing
// users transparently migrate on first read (handled by migrateLegacy below).
const GLOBAL_STORAGE        = "forge.global.dat"
const LEGACY_GLOBAL_STORAGE = "opencode.global.dat"
const LOCAL_PREFIX          = "forge."
// LOCAL_PREFIX_LEGACY is kept so quota-eviction (`evict()`) still recognises
// pre-rename keys when freeing space — without it, leftover `opencode.*` keys
// from older sessions would survive eviction sweeps until manual cleanup.
const LOCAL_PREFIX_LEGACY   = "opencode."
...
function evict(storage: Storage, keep: string, value: string) {
  ...
  for (const index of indexes) {
    ...
    if (!name) continue
    // Recognise both the current `forge.*` prefix and the pre-rename
    // `opencode.*` prefix so legacy entries are reclaimed under quota
    // pressure instead of squatting on bytes we can't free.
    if (!name.startsWith(LOCAL_PREFIX) && !name.startsWith(LOCAL_PREFIX_LEGACY)) continue
...
function workspaceStorage(dir: string) {
  ...
  return `forge.workspace.${head}.${sum}.dat`
}

// Returns the pre-rename `opencode.workspace.*` storage name(s) for the same
// directory — used as a legacy migration source so existing users don't lose
// per-project state (model selection, terminal sessions, VCS cache, …)
// after the namespace flip. Returns undefined when there's nothing to migrate.
...
function legacyWorkspaceStorage(dir: string) {
  ...
  // Add the pre-rename namespace for both canonical and raw `dir` forms so
  // users that picked up Forge before the rename keep their per-project state.
  const legacyCanonical = legacyOpencodeWorkspaceStorage(pathKey(dir))
  result.add(legacyCanonical)
  const legacyRaw = legacyOpencodeWorkspaceStorage(dir)
  if (legacyRaw !== legacyCanonical) result.add(legacyRaw)
  ...
  if (drive) {
    ...
    const legacyBackslash = legacyOpencodeWorkspaceStorage(key.replaceAll("/", "\\"))
    result.add(legacyBackslash)
  }
...
export const Persist = {
  global(key: string, legacy?: string[]): PersistTarget {
    // Read from `forge.global.dat` first; on miss the migration path checks
    // the pre-rename `opencode.global.dat` and copies into the current store.
