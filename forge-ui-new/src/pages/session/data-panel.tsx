...
export function DataPanel() {
  ...
  createEffect(() => { void projectId(); refreshTables() })
      ...
      migratePoll = setInterval(async () => {
        ...
        try {
          ...
          if (next.status === "succeeded" || next.status === "failed") {
            ...
            if (next.status === "succeeded") await refreshTables()
          }
        } catch { /* keep polling */ }
      }, 1000)
    } catch (err) {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ "font-weight": 600 }}>
          Data <span style={{ opacity: 0.6, "font-weight": 400, "margin-left": "8px" }}>
            sqlite · data.db
          </span>
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          <button onClick={() => setSqlOpen(s => !s)} style={btnStyle()}>SQL</button>
          <button onClick={() => setShowMigrate(true)} style={btnPrimaryStyle()}>
            Migrate to Supabase
          </button>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flex: 1, "min-height": 0 }}>
        ...
        <div style={{
          ...
          <Show when={tables().length === 0 && !loading()}>
            <div style={{ padding: "12px", opacity: 0.6, "font-size": "12px" }}>
              No tables yet. Ask the agent to add some.
            </div>
          </Show>
          <For each={tables()}>{(t) => (
