...
export default function Layout(props: ParentProps) {
  ...
  /**
   * Permanently delete a session — DESTRUCTIVE.
   *
   * Mirrors archiveSession's local-state surgery (splice from the store,
   * navigate to the next/prev or to the empty session route) so the sidebar
   * doesn't show a stale row while the server roundtrip completes.
   *
   * Why this isn't merged with archiveSession: archive is reversible (toggle
   * the timestamp); delete is not. We don't share the code path so a future
   * change to one can't accidentally widen the blast radius of the other.
   */
  async function deleteSession(session: Session) {
    ...
    // Optimistic splice — keep the UI snappy. Roll back on error.
    setStore(
      produce((draft) => {
        const match = Binary.search(draft.session, session.id, (s) => s.id)
        if (match.found) draft.session.splice(match.index, 1)
    ...
    try {
      ...
      // Roll back by re-inserting at the original index
      setStore(
        produce((draft) => {
          if (!draft.session.find((s) => s.id === session.id)) {
            draft.session.splice(index, 0, session)
          }
        }),
      )
      return
    ...
    return (
      <div class="relative bg-v2-background-bg-deep flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
        ...
        <main
          ...
          classList={{
            "m-2 rounded-[10px] shadow-[var(--v2-elevation-raised)] overflow-hidden": !!params.id || !params.dir,
          }}
        >
        ...
        {/* Forge: perf HUD hidden by default. Set VITE_FORGE_SHOW_DEBUG=1 to re-enable. */}
        {import.meta.env.DEV && import.meta.env.VITE_FORGE_SHOW_DEBUG === "1" && <DebugBar />}
        <Toast.Region />
      </div>
    )
  }
