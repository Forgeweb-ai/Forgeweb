...
export const DESKTOP_MENU: DesktopMenu[] = [
  ...
  },
  {
    ...
    label: "Help",
    items: [
      // Doc / community / issue tracker URLs intentionally removed — they
      // pointed at the upstream runtime's brand (opencode.ai/docs, the
      // anomalyco/opencode GitHub org, the runtime's Discord) which leaks
      // Forge internals per the IDENTITY GUARDRAIL in
      // forge-opencode-config/AGENTS.md. Re-add when Forge has its own
      // domain + repo + community (blocked on the rename per LAUNCH_PLAN §6a).
      { type: "item", label: "Export Logs...", command: "logs.export" },
    ],
  },
]
