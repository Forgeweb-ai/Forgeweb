...
describe("persist localStorage resilience", () => {
  ...
  test("workspace storage sanitizes Windows filename characters", () => {
    ...
    // Namespaces moved from `opencode.workspace.*` to `forge.workspace.*`.
    expect(result).toStartWith("forge.workspace.")
    expect(result.endsWith(".dat")).toBeTrue()
    expect(/[:\\/]/.test(result)).toBeFalse()
  })

  test("workspace target keeps raw path storage AND pre-rename opencode namespace as legacy fallbacks", () => {
    const target = Persist.workspace("C:\\Users\\foo", "vcs")

    expect(target.storage).toBe(persistTesting.workspaceStorage("C:/Users/foo"))
    // legacyStorageNames now includes:
    //   1. The raw-path forge namespace (pre-pathKey-normalization).
    //   2. The pre-rename opencode canonical namespace.
    //   3. The pre-rename opencode raw-path namespace.
    // Order is insertion order from legacyWorkspaceStorage's Set.
    expect(target.legacyStorageNames).toContain(persistTesting.workspaceStorage("C:\\Users\\foo"))
    expect(target.legacyStorageNames).toContain(`opencode.workspace.${persistTesting.workspaceStorage("C:/Users/foo").slice("forge.workspace.".length)}`)
    expect(target.legacyStorageNames).toContain(`opencode.workspace.${persistTesting.workspaceStorage("C:\\Users\\foo").slice("forge.workspace.".length)}`)
  })

  test("workspace target keeps backslash storage and opencode namespace as fallback for normalized Windows paths", () => {
    const target = Persist.workspace("C:/Users/foo", "vcs")

    expect(target.storage).toBe(persistTesting.workspaceStorage("C:/Users/foo"))
    // For a slash-normalized input the raw form is identical to canonical so
    // there's no "raw forge" entry — only the opencode canonical, opencode raw,
    // and the backslash forge variant from the drive-letter heuristic.
    expect(target.legacyStorageNames).toContain(persistTesting.workspaceStorage("C:\\Users\\foo"))
    expect(target.legacyStorageNames).toContain(`opencode.workspace.${persistTesting.workspaceStorage("C:/Users/foo").slice("forge.workspace.".length)}`)
  })
  ...
  test("removes legacy workspace storage when removing persisted target", () => {
    ...
    // Seed current storage + every fallback (raw forge + pre-rename opencode
    // namespaces) so we can confirm removePersisted cleans them all.
    storage.setItem(`${target.storage}:${target.key}`, '{"value":1}')
    for (const legacy of target.legacyStorageNames ?? []) {
      storage.setItem(`${legacy}:${target.key}`, '{"value":2}')
    }
    ...
    for (const legacy of target.legacyStorageNames ?? []) {
      expect(storage.getItem(`${legacy}:${target.key}`)).toBeNull()
    }
  })
})
