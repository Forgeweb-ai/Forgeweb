...
// Forge wordmark replaces opencode's `Logo` on the home-page background.
// Aliased so the JSX call site stays unchanged.
import { ForgeLogo as Logo } from "@/components/forge-logo"
...
import { setPendingAttachments, type PendingAttachment } from "@/pages/session/handoff"
...
const FORGE_HOME_STYLES = `
  ...
  .fh-prompt.is-dragover {
    border-color: var(--accent, oklch(0.62 0.155 45)) !important;
    background: var(--accent-soft, rgba(216, 90, 48, 0.08)) !important;
  }
  .fh-prompt-dropzone {
    position: absolute;
    inset: 6px;
    ...
    border: 1.5px dashed var(--accent, oklch(0.62 0.155 45));
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    gap: 8px;
    color: var(--accent, oklch(0.62 0.155 45));
    font-size: 13px;
    font-weight: 600;
    z-index: 10;
    pointer-events: none;
  ...
  .fh-prompt-dropzone svg { width: 22px; height: 22px; }
  [data-color-scheme="dark"] .fh-prompt.is-dragover {
    background: rgba(216, 90, 48, 0.12) !important;
  }
  [data-color-scheme="dark"] .fh-prompt-dropzone {
  ...
  .fh-attach-chip {
    ...
    border: 1px solid var(--hair, #e6dfd0);
    font-size: 12px;
    color: var(--ink-2, #2b2a25);
  }
  .fh-attach-thumb {
    width: 28px; height: 28px; border-radius: 5px; object-fit: cover; flex: none;
  }
  .fh-attach-icon {
  ...
  .fh-attach-remove {
    ...
    border-radius: 4px;
    color: var(--muted, #8a8175); cursor: pointer;
    flex: none;
    transition: background .12s ease, color .12s ease;
  }
  .fh-attach-remove:hover { background: rgba(21,20,15,0.08); color: var(--ink, #15140f); }
  .fh-attach-remove svg { width: 10px; height: 10px; }
  [data-color-scheme="dark"] .fh-attach-chip {
    background: rgba(255,255,255,0.05);
    border-color: #2c2920;
    color: #d8d0bd;
  }
  [data-color-scheme="dark"] .fh-attach-remove:hover {
    background: rgba(255,255,255,0.08); color: #f5efe0;
  }
  .fh-prompt-send {
    margin-left: auto;
...
function ForgeHome() {
  ...
  // ── Pending attachments (carry to session on submit) ─────────────────
  // Files the user attached on the home composer. Stored as base64 dataUrls
  // because raw File objects can't survive a route navigation. On submit we
  // hand them off to the session via setPendingAttachments, keyed by the
  // newly created project's workspace_path. The session page reads them on
  // mount (when ?from=home) and injects them into the composer's prompt as
  // ImageAttachmentPart entries (Phase 2 — see TODO in createFromPrompt).
  const [attachedFiles, setAttachedFiles] = createSignal<PendingAttachment[]>([])
  ...
  /**
   * Shared file-ingestion pipeline used by both the + button input and the
   * drag-and-drop drop handler. Filters by size, converts each file to a
   * base64 dataUrl, and appends the survivors to the attachedFiles signal.
   * Pure async work — no DOM mutation here, the caller decides UI state.
   */
  async function processIncomingFiles(files: File[]) {
    ...
    for (const file of files) {
      if (file.size > MAX_ATTACHMENT_BYTES) {
        console.warn(`Skipping ${file.name}: ${file.size} bytes exceeds ${MAX_ATTACHMENT_BYTES}`)
        continue
      }
    ...
    if (additions.length > 0) {
      setAttachedFiles([...attachedFiles(), ...additions])
    }
  }

  async function handleAttachInput(input: HTMLInputElement) {
  ...
  // ── Drag-and-drop on the home composer ───────────────────────────────────
  // Same UX as the in-session composer: dragover shows a highlight overlay,
  // dropped files run through the shared processIncomingFiles pipeline.
  //
  // dragCounter is needed because dragenter/dragleave fire for every child
  // element the cursor crosses. We can't just toggle on enter/leave or the
  // highlight flickers as the cursor moves between the textarea, the chip
  // strip, and the action row. Counting net enters keeps the highlight
  // stable for the entire drag session.
  ...
  // in-session composer (useLocal) already reads/writes the SAME field, so
  // routing the home dropdown through it too makes both surfaces agree in both
  // directions: a change in chat shows up here on the next home visit, and a
  // change here is what the next session loads. Fetched once per mount (home
  // remounts on navigation, so this is the freshest value) — flat cost, one GET
  // per page load. lastPatchedKey dedups so repeat clicks don't spam PATCH.
  ...
  }).catch(() => { /* offline / unauthed — fall back to local cache + defaults */ })

  const patchPrimaryModel = (item: ModelKey) => {
    const key = `${item.providerID}/${item.modelID}`
    if (key === lastPatchedKey) return
    lastPatchedKey = key
    void forge.updateSettings({ primary_model: key }).catch(() => {
      // Network blip — leave lastPatchedKey set so repeat clicks don't spam the
      // endpoint; the next genuine change re-attempts.
    })
  }
  ...
  const homeModelState = {
    ...
    current: () => {
      ...
      // forge-server primary_model is the single source of truth so this
      // dropdown and the in-session composer (useLocal) always agree, in both
      // directions. Resolved only once the registry has the model and its
      // provider is connected.
      const server = serverPrimaryModel()
      if (server && validModel(server)) {
        return models.find(server)
      }

      // Locally-cached key — instant first paint before getSettings resolves,
      ...
      // Legacy global opencode config — kept as a secondary so a stock/older
      // session that only wrote config.model still resolves.
    ...
    set(item: ModelKey | undefined, options?: { recent?: boolean }) {
      ...
      setServerPrimaryModel(item)            // optimistic — dropdown reflects it now
      localStorage.setItem("forge_home_model_v2", JSON.stringify(item))
      models.setVisibility(item, true)
      if (options?.recent) {
        models.recent.push(item)
      }
      // Single source of truth: persist to forge-server (user_settings.primary_model)
      // so the in-session composer (useLocal) loads the same model. Deduped,
      // fire-and-forget — no UI block on network.
      patchPrimaryModel(item)
      // Mirror into the global opencode config too so a stock/legacy session that
      // still reads config.model stays in sync. Secondary — non-fatal.
      const globalModelStr = `${item.providerID}/${item.modelID}`
      globalSync.updateConfig({ model: globalModelStr }).catch(() => {
        // non-fatal — BE + local state are still updated
      })
    },
  ...
  async function createFromPrompt() {
    ...
    try {
      ...
      // Park any attached files in the in-memory handoff store keyed by the
      // project's workspace_path. The session page (Phase 2 — pending) will
      // read + clear this on first mount when ?from=home is present.
      const pending = attachedFiles()
      if (pending.length > 0) {
        setPendingAttachments(project.workspace_path, pending)
        setAttachedFiles([])
      }
      ...
      <div class="fh-root">
        ...
        <main class="fh-main">
          ...
          <div class="fh-hero">
            ...
            <div
              class={`fh-prompt ${isDraggingOver() ? "is-dragover" : ""}`}
              onDragEnter={handleDragEnter}
              onDragOver={handleDragOver}
              onDragLeave={handleDragLeave}
              onDrop={(e) => void handleDrop(e)}
            >
              {/* Drop overlay — only painted while a file drag is over the
                  composer. pointer-events: none on the overlay so the drop
                  itself still hits the composer below. */}
              <Show when={isDraggingOver()}>
                <div class="fh-prompt-dropzone" aria-hidden="true">
              ...
              {/* Attached files — chip strip above the action row */}
              <Show when={attachedFiles().length > 0}>
                <div class="fh-attach-list">
                  <For each={attachedFiles()}>
                    {(file, i) => (
                      <div class="fh-attach-chip" title={`${file.name} · ${Math.round(file.size / 1024)} KB`}>
                        <Show
                          when={file.mime.startsWith("image/")}
                          fallback={
                            <svg viewBox="0 0 20 20" class="fh-attach-icon" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                              <path d="M5 3h8l4 4v10a1 1 0 01-1 1H5a1 1 0 01-1-1V4a1 1 0 011-1z" />
                              <path d="M13 3v4h4" />
                            </svg>
                          }
                        >
                          <img class="fh-attach-thumb" src={file.dataUrl} alt={file.name} />
                        </Show>
                        <span class="fh-attach-name">{file.name}</span>
                        <button
                          type="button"
                          class="fh-attach-remove"
              ...
              <div class="fh-prompt-row">
                {/* Hidden native file input — triggered by the + chip */}
                <input
                  ref={attachInputRef}
                  type="file"
                  multiple
                  ...
                  style={{ display: "none" }}
                  onChange={(e) => void handleAttachInput(e.currentTarget)}
                />
                <button
                  type="button"
                  class="fh-prompt-chip icon interactive"
                  title="Attach files"
