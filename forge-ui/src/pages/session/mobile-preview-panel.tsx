...
export function MobilePreviewPanel() {
  ...
  // Visual-edit select mode — mirrors the desktop side-panel. See
  // session-side-panel.tsx for the design rationale (cascade safety,
  // token cost shape, etc.); the wiring is identical, only the local
  // iframeRef differs.
  const [selectActive, setSelectActive] = createSignal(false)
  const unsubscribeSelectPick = onSelectPick((pick) => {
    ...
    showToast({
      ...
      description: "Open chat, type your change, and send.",
    })
  })
  onCleanup(unsubscribeSelectPick)
