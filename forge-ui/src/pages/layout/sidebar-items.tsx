      ...
      >
        <div class="flex min-w-0 items-center gap-1">
          ...
          <Show when={!props.level}>
            <div
              class="shrink-0 flex items-center gap-0.5 overflow-hidden transition-[width,opacity]"
              classList={{
            ...
            >
              ...
              <Tooltip value={language.t("common.delete")} placement="top">
                <IconButton
                  icon="trash"
                  variant="ghost"
                  class="size-6 rounded-md text-icon-critical-base hover:text-icon-critical-base"
                  aria-label={language.t("common.delete")}
                  onClick={(event) => {
                    event.preventDefault()
                    event.stopPropagation()
                    // Confirm inline — no extra dialog component needed for a
                    // single-line action. native confirm() ships free.
                    if (typeof window !== "undefined" && !window.confirm(
                      `Permanently delete "${sessionTitle(props.session.title) || props.session.id}"? This cannot be undone.`,
                    )) return
