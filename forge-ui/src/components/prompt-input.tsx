...
import type { FixPrefillPayload } from "@/utils/fix-this-screen"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  // Forge slash-menu policy:
  // - Built-ins: whitelist only user-facing session controls. Everything else
  //   (model/agent/terminal/mcp/open/etc.) is reachable via the UI; surfacing
  //   them as slash commands clutters the menu for non-developer end users.
  // - Custom commands: drop `skill` source (skills auto-trigger inside the
  //   agent via description match — they are NOT user-invokable shortcuts) and
  //   drop opencode-internal dev commands (`init`, `review`,
  //   `customize-opencode`) that aren't meaningful in Forge sessions.
  // Hiding from the menu does NOT disable the underlying capability: skills
  // still load inside the opencode agent, and power users can still type a
  // dropped slash literally — submit.ts resolves it via sync.data.command.
  const FORGE_BUILTIN_SLASH_ALLOW = new Set(["compact", "new", "undo", "fork", "share"])
  const FORGE_CUSTOM_SLASH_DENY = new Set(["init", "review", "customize-opencode"])
  ...
  const slashCommands = createMemo<SlashCommand[]>(() => {
    const builtin = command.options
      .filter(
        (opt) =>
          !opt.disabled &&
          !opt.id.startsWith("suggested.") &&
          opt.slash &&
          FORGE_BUILTIN_SLASH_ALLOW.has(opt.slash),
      )
      .map((opt) => ({
        id: opt.id,
        trigger: opt.slash!,
    ...
    const custom = sync.data.command
      .filter((cmd) => cmd.source !== "skill" && !FORGE_CUSTOM_SLASH_DENY.has(cmd.name))
      .map((cmd) => ({
        id: `custom.${cmd.name}`,
        trigger: cmd.name,
        title: cmd.name,
  ...
  const [searchParams, setSearchParams] = useSearchParams<{ prompt?: string; model?: string }>()

  createEffect(() => {
    if (!prompt.ready() || !local.model.ready() || useProviders().connected().length === 0 || !local.model.current() || !local.agent.current() || params.id) return
    const text = searchParams.prompt
    if (!text) return

    // Prevent duplicate submission on back/forward history navigation in this tab
    const key = `forge_submit_${params.dir ?? ""}_${text}_${searchParams.model ?? ""}`
    if (sessionStorage.getItem(key)) {
      setSearchParams({ ...searchParams, prompt: undefined, model: undefined }, { replace: true })
      return
    }
    ...
    if (modelParam) {
      const [providerID, modelID] = modelParam.split("/")
      if (providerID && modelID) {
        local.model.set({ providerID, modelID }, { recent: true })
      }
    }

    // Clear search params immediately to prevent loops
    setSearchParams({ ...searchParams, prompt: undefined, model: undefined }, { replace: true })
  ...
  /**
   * Listen for `forge:prefill-prompt` — fired by the preview "Fix this"
   * button (utils/fix-this-screen.ts) and the runtime-errors banner fallback.
   *
   * Why we DON'T auto-submit here (unlike the searchParams path above):
   *   The user explicitly asked for a "review before send" loop in the
   *   founder review — they want to add detail or pick a model before the
   *   token spend. Banner copies to clipboard for the same reason.
   *
   * If we ever want a one-click auto-send variant, the event payload can
   * grow a `submit?: boolean` flag; do not flip the default.
   */
  const handlePrefillPrompt = (ev: Event) => {
    if (!prompt.ready()) return
    const detail = (ev as CustomEvent<FixPrefillPayload>).detail
    if (!detail || typeof detail.text !== "string") return

    const parts: ContentPart[] = [
      { type: "text", content: detail.text, start: 0, end: detail.text.length },
    ]
    if (detail.image && detail.image.dataUrl) {
      parts.push({
        type:     "image",
        id:       uuid(),
        filename: detail.image.filename || "preview.jpg",
        mime:     detail.image.mime     || "image/jpeg",
        dataUrl:  detail.image.dataUrl,
      })
    }

    prompt.set(parts, detail.text.length)
    requestAnimationFrame(() => {
      editorRef?.focus()
      setCursorPosition(editorRef, detail.text.length)
    })
  }

  onMount(() => {
    window.addEventListener("forge:prefill-prompt", handlePrefillPrompt as EventListener)
  })
  onCleanup(() => {
    window.removeEventListener("forge:prefill-prompt", handlePrefillPrompt as EventListener)
  })
            ...
            >
              <div class="relative max-h-[180px] overflow-y-auto no-scrollbar" ref={(el) => (scrollRef = el)}>
                <div
                  ...
                  classList={{
                    "select-text": true,
                    // text-v2-text-text-base for typed text (darker / readable);
                    // the placeholder div below this still uses -faint so empty
                    // state looks correctly de-emphasised.
