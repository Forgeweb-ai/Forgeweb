...
// True when this build is running inside Forge (VITE_API_URL points at the
// forge-server). When true, API-key writes from the Connect dialog must go
// through forge-server's per-user vault, NOT opencode's shared on-disk
// auth.json — see provider_routes.py + the X-Forge-Auth proxy.
const IN_FORGE_MODE = !!(import.meta.env.VITE_API_URL as string | undefined)
...
export function DialogConnectProvider(props: { provider: string }) {
  ...
  function ApiAuthView() {
    ...
    async function handleSubmit(e: SubmitEvent) {
      ...
      if (IN_FORGE_MODE && forge) {
        // Forge BYOK path: persist to forge-server's per-user encrypted vault
        // (user_provider_keys). The opencode proxy reads from there per
        // request and injects via X-Forge-Auth, so the key NEVER touches the
        // shared on-disk auth.json. Going through globalSDK.client.auth.set
        // here would write to that shared file and leak the key to every
        // other user on the same opencode process.
        await forge.setProviderKey(props.provider, apiKey, undefined)
      } else {
