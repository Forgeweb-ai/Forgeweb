...
export default function FileTree(props: {
  ...
  // Files/dirs that should never be visible in any file tree view.
  // node_modules is gitignored and handled by the runner container's Docker
  // volume — opencode writes it to the host workspace during npm install but
  // the user never needs to see or interact with it.
  const TREE_HIDDEN = new Set(["AGENTS.md", "CLAUDE.md", "opencode.json"])

  const nodes = createMemo(() => {
    const all = file.tree
      .children(props.path)
      .filter((n) => !TREE_HIDDEN.has(n.name) && !n.ignored)
    const current = filter()
                ...
                >
                  <Collapsible.Trigger>
                    ...
                    >
                      ...
                      <div class="size-4 flex items-center justify-center text-amber-500/80 shrink-0 ml-0.5">
                        <svg class="size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
