...
export const DialogManageModels: Component = () => {
  ...
  // Build a flat list of visible models: value = "providerID/modelID".
  // We then ALWAYS prepend the currently-saved design_model as an option,
  // even when the underlying provider is hidden or disconnected. Without
  // this, `<select value=X>` with no matching `<option value=X>` falls
  // back to displaying the first option's label, which makes the user
  // think their saved choice "keeps changing automatically" — the value
  // in the DB is still correct, the dropdown just can't show it. The
  // synthesized entry preserves the user's intent visually and lets them
  // explicitly re-pick if they want to switch.
  const designModelOptions = () => {
    ...
    const saved = settings()?.design_model
    if (saved && !base.some((o) => o.value === saved)) {
      // Try to find a friendly label from the full (unfiltered) model
      // registry; fall back to the raw id so the user always sees SOMETHING
      // meaningful instead of an empty select.
      const full = models.list().find((m) => `${m.provider.id}/${m.id}` === saved)
      const friendly = full ? `${full.provider.name} / ${full.name}` : saved
      base.unshift({ value: saved, label: `${friendly} (saved)` })
    }
    return base
  }
