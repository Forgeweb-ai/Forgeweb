...
export const SettingsDesignModel: Component = () => {
  ...
  // True when the user has a saved design_model but it isn't represented
  // anywhere in the visible model list (provider disconnected or model
  // hidden). Without surfacing this, the radio list shows nothing as
  // selected and the user thinks the setting "keeps changing" — the value
  // is in the DB, it just has no UI handle. We render a clear notice with
  // the saved id and a Clear action so the user can recover deliberately.
  const savedMissingFromList = () => {
    const saved = selectedValue()
    if (!saved) return false
    return !models.list().some((m) => `${m.provider.id}/${m.id}` === saved)
  }
  ...
  return (
    <div class="flex flex-col h-full overflow-y-auto no-scrollbar px-4 pb-10 sm:px-10 sm:pb-10">
      ...
      <div class="flex flex-col gap-3 pt-6 pb-6 max-w-[720px]">
        ...
        {/* Saved-but-unavailable notice. Surfaces the persisted value when
            the user can't see it in the list below — the cause of "the
            setting keeps disappearing" complaints. */}
        <Show when={savedMissingFromList()}>
          <div class="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-13-regular text-text-base flex items-center justify-between gap-3">
            <span>
              Saved: <code class="text-12-regular text-text-strong">{selectedValue()}</code>
              <span class="ml-1 text-text-weak">(provider hidden — pick again below to switch)</span>
            </span>
          </div>
        </Show>
      </div>
