...
function readStoredLocale() {
  ...
  // Direct read — used BEFORE LanguageProvider mounts so we can preload the
  // right dictionary file at module-eval time. The Persist namespaces moved
  // from `opencode.*` to `forge.*`; we prefer the new key but transparently
  // fall back to the old one so a returning user still gets their language
  // applied on first paint (the next Persist write will move it forward).
  try {
    const raw =
      localStorage.getItem("forge.global.dat:language") ??
      localStorage.getItem("opencode.global.dat:language")
    if (!raw) return
