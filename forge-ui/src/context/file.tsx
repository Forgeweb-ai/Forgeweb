    ...
    const tree = createFileTreeStore({
      ...
      list: (dir) =>
        sdk.client.file.list({ path: dir }).then((x) => {
          const nodes = x.data ?? []
          // Hide internal Forge/AI-tool files from the file tree at the workspace root.
          const HIDDEN_AT_ROOT = new Set([".opencode", "forge.json"])
          if (dir === "") return nodes.filter((n) => !HIDDEN_AT_ROOT.has(n.name))
          return nodes
        }),
      onError: (message) => {
        showToast({
